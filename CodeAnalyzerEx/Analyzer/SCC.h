...
*  Package Operations:
*  ===================
*  SCC uses TypeTable, Dependency Table package to find the strongly connected components using Tarjan's Algorithm (reference geeksforgeeks).
*
...
private:
	...
	size_t numberofVertices = 0;   //total number of vertices present
...
inline void SCC::startgraph()
{
	...
	for (Key key : keys)
	{
		std::vector<std::string> childvec = db.value(key).child;
