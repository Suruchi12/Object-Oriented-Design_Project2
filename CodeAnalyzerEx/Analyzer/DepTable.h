	...
	if (filespec != "") {
		...
		try {
			...
			if (file.good()) {
				...
				depTable.insert(std::make_pair(fname, depVector));
			}
			else{
				cout << "Cannot open the file!";
				exit(0);} std::cout << std::endl;    
			for (auto depTableit = depTable[fname].begin(); depTableit != depTable[fname].end(); ++depTableit) {
				cout << std::left << std::setw(30) << fname << *depTableit <<endl;  //print the dependency table
			}
		}
