...
//void CodeAnalysisExecutive::myExec()
//{
...
//	dt.dbtoxml(argv[argc - 1]);	      //writes the data to an XML File
//}
...
{using namespace CodeAnalysis;
  ...
  try {bool succeeded = exec.ProcessCommandLine(argc, argv);
	...
	NoSqlDb<string> db1 = dt.getdb();			//gets the dependency database
	SCC scc(exec.temp.size(), db1);  //constructor for strongly connected components
	scc.startgraph();           //makes a graph for scc
	scc.sccomponent();			//creates all strongly connected components
	dt.printdatabase(); //print the dependency table
	dt.dbtoxml(argv[argc - 1]);					//writes the data to an XML File
